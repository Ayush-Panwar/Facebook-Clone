...
exports.register = require("mongoose");
  ...
  try {
    if (req.user.id != req.params.id) {
      ...
      if (
        ...
        await receiver.updateOne({ $push: { followers: sender._id } });
        await sender.updateOne({ $push: { following: sender._id } });
  ...
  try {
    if (req.user.id != req.params.id) {
      ...
      if (
        ...
        await sender.update({
          $push: { friends: receiver._id, followers: receiver.id },
        });
        await receiver.updateOne({ $pull: { requests: sender._id } });
